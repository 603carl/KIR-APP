export const KENYA_SUB_COUNTIES: Record<string, string[]> = {
    ...
    'Nairobi City': ['Dagoretti North', 'Dagoretti South', 'Embakasi Central', 'Embakasi East', 'Embakasi North', 'Embakasi South', 'Embakasi West', 'Kamukunji', 'Kasarani', 'Kibra', "Lang'ata", 'Makadara', 'Mathare', 'Roysambu', 'Ruaraka', 'Starehe', 'Westlands'],
    ...
    'Tharaka-Nithi': ['Chiakariga', "Igambango'mbe", 'Chuka', 'Maara', 'Muthambi', 'Tharaka North', 'Tharaka South'],
...
export function fallbackSubCountiesForCounty(county: string): string[] {
    const normalized = county.trim();
    if (!normalized) return [];
    return KENYA_SUB_COUNTIES[normalized]
        ?? KENYA_SUB_COUNTIES[normalized.replace(/\s+County$/i, '')]
        ?? [];
}
